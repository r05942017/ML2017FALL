...
w=np.asmatrix(mean_with50-mean_no50)*np.asmatrix(pinv(covariance))		#1D matrix
b=-0.5*np.asmatrix(mean_with50)*np.asmatrix(pinv(covariance))*np.asmatrix(mean_with50).T+0.5*np.asmatrix(mean_no50)*np.asmatrix(pinv(covariance))*np.asmatrix(mean_no50).T+np.log(len(data_with50)/len(data_no50))
